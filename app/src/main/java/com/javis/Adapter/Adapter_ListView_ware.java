...
public class Adapter_ListView_ware extends BaseAdapter {
	...
	public Adapter_ListView_ware(Context context, List<Goods> list) {
	...
	public int getCount() {
		return list.size();
		//(arrayList != null && arrayList.size() == 0) ? 30: arrayList.size()
	}
	...
	public View getView(int position, View currentView, ViewGroup arg2) {
		...
		holderView.iv_pic.setImageResource(list.get(position).getImage());
		holderView.tv_name.setText(list.get(position).getPro());
		holderView.tv_price.setText(list.get(position).getPrice()+"");
		return currentView;
	}
